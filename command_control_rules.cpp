...
// T1105 - Download File To Potentially Suspicious Directory Via Wget
// SELECT * FROM bpf_process_events WHERE path LIKE '%/wget%' AND cmdline LIKE '%--output-document%' AND cmdline LIKE '%/tmp/%';
...
bool download_file_to_potentially_suspicious_directory_via_wget(const ProcessEvent &process_event, Event &rule_event)
{
    ...
    if (path.find("/wget") != std::string::npos && cmdline.find("--output-document") != std::string::npos && cmdline.find("/tmp/") != std::string::npos)
    {
        std::stringstream ss;

        ss << "Detected the use of wget to download content to a suspicious directory.";
        rule_event.metadata = ss.str();
...
// T1219 - Potential Linux Amazon SSM Agent Hijacking
// SELECT * FROM process_events WHERE path LIKE '%/amazon-ssm-agent%' AND cmdline LIKE '%-register%' AND cmdline LIKE '%-code%' AND cmdline LIKE '%-id%' AND cmdline LIKE '%-region%';
...
bool potential_linux_amazon_ssm_agent_hijacking(const ProcessEvent &process_event, Event &rule_event)
{
    ...
    if (path.find("/amazon-ssm-agent") != std::string::npos && cmdline.find("-register") != std::string::npos && cmdline.find("-code") != std::string::npos && cmdline.find("-id") != std::string::npos && cmdline.find("-region") != std::string::npos)
    {
        std::stringstream ss;

        ss << "Potential Linux Amazon SSM Agent Hijacking";
        rule_event.metadata = ss.str();
...
// T1071.001 - Suspicious Curl Change User Agents
// SELECT * FROM process_events WHERE path LIKE '%/curl%' AND (cmdline LIKE '% -A %' OR cmdline LIKE '% --user-agent %');
...
bool suspicious_curl_change_user_agents(const ProcessEvent &process_event, Event &rule_event)
{
    ...
    if (path.find("/curl") != std::string::npos && (cmdline.find(" -A ") != std::string::npos || cmdline.find(" --user-agent ") != std::string::npos))
    {
        std::stringstream ss;

        ss << "Suspicious Curl Change User Agents";
        rule_event.metadata = ss.str();
