...
bool suspicious_curl_file_upload(const ProcessEvent &process_event, Event &rule_event)
{
    if (process_event.entry.path.find("/curl") != std::string::npos and process_event.entry.syscall.find("connect") != std::string::npos)
    {
        std::stringstream ss;

        ss << "[" << process_event.entry.file_path << " (pid: " << process_event.entry.pid << ")]T1567 - Suspicious Curl File Upload";
        rule_event.metadata = ss.str();
