            ...
            for f, value_list in ProcessEventProcessor("Linux").items():
                if len(value_list) == 4:
                    ...
                    if boolean == True:
                        ...
                        if "process_tree" in local_dict and not local_dict["process_tree"]:
                                a = []
                        else:
                    ...
                    try:
                        ...
                        # print(f" bpf_process_events: JSON parsing error: {e}")  # Log the specific error
                        func = value_list[4]  # Use the original string as a fallback
                    exec(func)
                    # sleep(2)
                    try:
                        ...
                        # Log the error and skip this particular call
                        # print(f"bpf_process_events: Skipping function {f} due to missing key: {e.args[0]}")
                        continue
            ...
            for f, value_list in ProcessEventProcessor("Linux").items():
                if len(value_list) == 4:
                    ...
                    try:
                        ...
                        # print(f"bpf_socket_events: JSON parsing error: {e}")  # Log the specific error
                        func = value_list[4]  # Use the original string as a fallback
                    exec(func)
                    # sleep(2)
                    try:
                        ...
                        if boolean == True:
                            ...
                            cursor.execute(sql, insert_sql)
                            conn.commit()
                    # Use the original string as a fallback
                    except KeyError as e:
                        # Log the error and skip this particular call
                        # print(f"Skipping function {f} due to missing key: {e.args[0]}")
                        continue
            ...
            for f, value_list in ProcessEventProcessor("Linux").items():
                if len(value_list) == 4:
                    ...
                    try:
                        ...
                        # print(f"bpf_file_events: JSON parsing error: {e}")  # Log the specific error
                        func = value_list[4]  # Use the original string as a fallback
                    exec(func)
                    # sleep(2)
                    try:
                        ...
                        # Log the error and skip this particular call
                        # print(f"bpf_file_events: Skipping function {f} due to missing key: {e.args[0]}")
                        continue
            cursor = conn.cursor()
            cursor.execute("Select * FROM bpf_file_events LIMIT 0")
            colnames = [desc[0] for desc in cursor.description]
            ...
            for f, value_list in ProcessEventProcessor("Windows").items():
                if len(value_list) == 4:
                    ...
                    try:
                        ...
                        # print(f"win_process_events: JSON parsing error: {e}")  # Log the specific error
                        func = value_list[4]  # Use the original string as a fallback
                    exec(func)
                    # sleep(2)
                    try:
                        ...
                        # Log the error and skip this particular call
                        # print(f"win_process_events: Skipping function {f} due to missing key: {e.args[0]}")
                        continue
            ...
            for f, value_list in ProcessEventProcessor("Windows").items():
                if len(value_list) == 4:
                    ...
                    try:
                        ...
                        # print(f"win_socket_events: JSON parsing error: {e}")  # Log the specific error
                        func = value_list[4]  # Use the original string as a fallback
                    exec(func)
                    # sleep(2)
                    try:
                        ...
                        # Log the error and skip this particular call
                        # print(f"win_socket_events: Skipping function {f} due to missing key: {e.args[0]}")
                        continue
            ...
            for f, value_list in ProcessEventProcessor("Windows").items():
                if len(value_list) == 4:
                    ...
                    try:
                        ...
                        # print(f"win_file_events: JSON parsing error: {e}")  # Log the specific error
                        func = value_list[4]  # Use the original string as a fallback
                    exec(func)
                    # sleep(2)
                    try:
                        ...
                        # Log the error and skip this particular call
                        # print(f"win_file_events: Skipping function {f} due to missing key: {e.args[0]}")
                        continue
                ...
                for f, value_list in processor.items():
                    if len(value_list) == 4:
                        ...
                        try:
                            ...
                            # print(f"chrome_ext: JSON parsing error: {e}")  # Log the specific error
                            func = value_list[4]  # Use the original string as a fallback
                        except KeyError as e:
                            # Log the error and skip this particular call
                            # print(f"chrome_ext: Skipping function {f} due to missing key: {e.args[0]}")
                            continue
                ...
                for f, value_list in processor.items():
                    if len(value_list) == 4:
                        ...
                        try:
                            ...
                            # print(f"Processes: JSON parsing error: {e}")  # Log the specific error
                            func = value_list[4]  # Use the original string as a fallback
                        except KeyError as e:
                            # Log the error and skip this particular call
                            # print(f"Processes: Skipping function {f} due to missing key: {e.args[0]}")
                            continue
