...
// T1056.001 - Input Capture: Keylogging - merge
// https://github.com/redcanaryco/atomic-red-team/blob/master/atomics/T1056.001/T1056.001.md#t1056001---input-capture-keylogging
...
// T1592.004 - Linux Recon Indicators
// SELECT * FROM bpf_process_events WHERE cmdline LIKE '% -name .htpasswd%' OR cmdline LIKE '% -perm -4000 %';
bool linux_recon_indicators(const ProcessEvent& process_event, Event& rule_event)
{
    if (process_event.entry.cmdline.find(" -name .htpasswd")!= std::string::npos or process_event.entry.cmdline.find(" -perm -4000 ")!= std::string::npos)
    {
            std::stringstream ss;

            ss << "Detected events with patterns found in commands used for reconnaissance on linux systems" ;
            rule_event.metadata = ss.str();
...
// T1564 - Mount Execution With Hidepid Parameter
// SELECT * FROM win_process_events WHERE (path LIKE '%/mount%' AND (cmdline LIKE '%hidepid=2%' OR cmdline LIKE '% -o %'));

bool mount_execution_with_hidepid_parameter(const ProcessEvent& process_event, Event& rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;

    if ((path.find("/mount")!= std::string::npos) && (cmdline.find("hidepid=2")!= std::string::npos || cmdline.find(" -o ")!= std::string::npos))
    {
            std::stringstream ss;
            ss << "Execution of the 'mount' command with 'hidepid' parameter to make invisible processes to other users from the system detected !" ;
            rule_event.metadata = ss.str();
            return true;
    }
    return false;
