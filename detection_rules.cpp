...
{
    if ((process_event.entry.cmdline.find("trap") != std::string::npos && process_event.entry.cmdline.find("sh")) && (process_event.entry.cmdline.find("SIGINT") || process_event.entry.cmdline.find("EXIT") != std::string::npos))
    {
...
bool scheduled_task_cron(const ProcessEvent &process_event, Event &rule_event)
{
    ...
    {
        if ((process_event.entry.cmdline.find("crontab") != std::string::npos) && (process_event.entry.cmdline.find("-l") != std::string::npos || process_event.entry.cmdline.find("-l") != std::string::npos || process_event.entry.cmdline.find("-e") != std::string::npos || process_event.entry.cmdline.find("-r") != std::string::npos || process_event.entry.cmdline.find("-v") != std::string::npos))
        {
            std::stringstream ss;
...
bool bash_history_commandLine(const ProcessEvent &process_event, Event &rule_event)
{
    if ((process_event.entry.cmdline.find("unset HISTFILE") != std::string::npos) or (process_event.entry.cmdline.find("export HISTFILESIZE=0") != std::string::npos) or (process_event.entry.cmdline.find("history -c") != std::string::npos) or (process_event.entry.cmdline.find("rm ~/.bash_history") != std::string::npos))
    {
        ...
        ss << "[" << process_event.entry.path << " (pid: " << process_event.entry.pid << ")] bash history cleared";
        rule_event.metadata = ss.str();
...
{
    if (process_event.entry.path == "/usr/bin/systemd-detect-virt" || process_event.entry.cmdline == "systemd-detect-virt")
    {
...
{
    if (process_event.entry.path == "/usr/sbin/dmidecode" && process_event.entry.cmdline == "dmidecode")
    {
...
bool impair_cmd_history_logging_mac_hist_control(const ProcessEvent &process_event, Event &rule_event)
{
    if ((process_event.entry.cmdline.find("export HISTCONTROL=ignoreboth") != std::string::npos) and (process_event.entry.cmdline.find("echo export \"HISTCONTROL=ignoreboth\" >> ~/.bash_profile") != std::string::npos))
    {
        ...
        ss << "[" << process_event.entry.path << " (pid: " << process_event.entry.pid << ")] T1562.003 - Linux HISTCONTROL";
        rule_event.metadata = ss.str();
...
{
    if (process_event.entry.cmdline.find("shred") != std::string::npos)
    {
...
{
    if (process_event.entry.file_path.find(".ssh/authorized_keys") != std::string::npos || process_event.entry.file_path.find("/etc/ssh/sshd_config") != std::string::npos)
    {
        std::stringstream ss;
...
{
    if (process_event.entry.cmdline.find("sudo") != std::string::npos && (process_event.entry.cmdline.find("sh -c") != std::string::npos) and (process_event.entry.cmdline.find("/etc/ld.so.preload") != std::string::npos))
    {
...
{
    if (process_event.entry.cmdline.find("systemctl")!= std::string::npos && process_event.entry.cmdline.find("falcon-sensor.service")!= std::string::npos && (process_event.entry.cmdline.find("stop")!= std::string::npos || process_event.entry.cmdline.find("disable")!= std::string::npos))
    {
...
{
    if ((process_event.entry.cmdline.find("ufw prepend deny from")!= std::string::npos) or (process_event.entry.cmdline.find("ufw status numbered")!= std::string::npos))
    {
...
{
    if ((process_event.entry.cmdline.find("echo")!= std::string::npos and process_event.entry.cmdline.find("/etc/ufw/user.rules")!= std::string::npos) or (process_event.entry.cmdline.find("user.rules")!= std::string::npos && process_event.entry.cmdline.find("grep")!= std::string::npos ))
    {
...
{
    if ((process_event.entry.cmdline.find("echo")!= std::string::npos and process_event.entry.cmdline.find("/etc/ufw/sysctl.conf")!= std::string::npos) or (process_event.entry.cmdline.find("grep")!= std::string::npos && process_event.entry.cmdline.find("/etc/ufw/sysctl.conf")!= std::string::npos))
    {
...
{
    if ((process_event.entry.cmdline.find("echo")!= std::string::npos and process_event.entry.cmdline.find("/etc/default/ufw")!= std::string::npos) or (process_event.entry.cmdline.find("grep")!= std::string::npos && process_event.entry.cmdline.find("default/ufw")!= std::string::npos))
    {
...
{
    if (process_event.entry.cmdline.find("tail")!= std::string::npos && process_event.entry.cmdline.find("/var/log/ufw.log")!= std::string::npos)
    {
...
{
    if (process_event.entry.cmdline.find("xxd") != std::string::npos && process_event.entry.cmdline.find(" -r ") != std::string::npos && process_event.entry.cmdline.find(" -p ") != std::string::npos )
    {
...
{
    if ((process_event.entry.cmdline.find("gcloud")!= std::string::npos && process_event.entry.cmdline.find("iam")!= std::string::npos && process_event.entry.cmdline.find("service-accounts")!= std::string::npos) && (process_event.entry.cmdline.find("create")!= std::string::npos || (process_event.entry.cmdline.find("keys")!= std::string::npos && process_event.entry.cmdline.find("create")!= std::string::npos)))
    {
...
{
    if (process_event.entry.cmdline.find("echo 0>")!= std::string::npos && process_event.entry.cmdline.find("/var/spool/mail/")!= std::string::npos)
    {
...
{
    if (process_event.entry.path.find("/usr/bin/at")!= std::string::npos && process_event.entry.cmdline.find("at")!= std::string::npos)
    {
...
{
    if (process_event.entry.cmdline.find("cat")!= std::string::npos && process_event.entry.cmdline.find("/etc/passwd")!= std::string::npos)
    {
...
{
    if (process_event.entry.cmdline.find("grep")!= std::string::npos && process_event.entry.cmdline.find("x:0:")!= std::string::npos && process_event.entry.cmdline.find(" /etc/passwd")!= std::string::npos)
    {
...
{
    if (process_event.entry.cmdline == "groups")
    {
...
{
    if ((process_event.entry.cmdline.find("tcpdump")!= std::string::npos && process_event.entry.cmdline.find("-nnni")!= std::string::npos) or (process_event.entry.cmdline.find("tshark")!= std::string::npos && process_event.entry.cmdline.find("-i")!= std::string::npos))
    {
...
{
    if (process_event.entry.cmdline.find("dscacheutil -q group") != std::string::npos and process_event.entry.cmdline.find("dscl . -list /Groups") != std::string::npos and process_event.entry.cmdline.find("groups") != std::string::npos and process_event.entry.cmdline.find("id") != std::string::npos and process_event.entry.cmdline.find("getent group") != std::string::npos and process_event.entry.cmdline.find("/etc/group") != std::string::npos)
    {
...
{
    if (process_event.entry.cmdline.find("7z") != std::string::npos && process_event.entry.cmdline.find("a") != std::string::npos && process_event.entry.cmdline.find("-p") != std::string::npos && process_event.entry.cmdline.find("-mhe") != std::string::npos)
    {
        std::stringstream ss;
...
{
    if (process_event.entry.cmdline.find("rsautl") != std::string::npos && process_event.entry.cmdline.find("-encrypt") != std::string::npos && process_event.entry.cmdline.find("-inkey") != std::string::npos)
    {
        std::stringstream ss;
...
{
    if ((process_event.entry.cmdline.find("dd") != std::string::npos and process_event.entry.cmdline.find("if") != std::string::npos and process_event.entry.cmdline.find("of") != std::string::npos) || (process_event.entry.path.find("\\cipher.exe") != std::string::npos && process_event.entry.cmdline.find("/w") != std::string::npos))
    {
        std::stringstream ss;
...
{
    if (process_event.entry.file_path.find("/etc/cron.allow") != std::string::npos or process_event.entry.file_path.find("/etc/cron.deny") != std::string::npos)
    {
...
{
    if (process_event.entry.cmdline.find("systemd-run") != std::string::npos and process_event.entry.cmdline.find("--user") != std::string::npos and process_event.entry.cmdline.find("/bin/sh -c") != std::string::npos)
    {
...
{
    if (process_event.entry.cmdline.find("rsync -r") != std::string::npos or (process_event.entry.cmdline.find("whois -h") != std::string::npos && process_event.entry.cmdline.find("-p") != std::string::npos))
...
// T1140 - Linux Base64 Encoded Pipe to Shell
// select * from bpf_process_events where cmdline like '%base64%' and (cmdline like '%bash%' or cmdline like '%sh%');

bool linux_base64_encoded_pipe_shell(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    if (cmdline.find("base64") != std::string::npos && (cmdline.find("bash") != std::string::npos || cmdline.find("sh") != std::string::npos))
    {
        std::stringstream ss;
        ss << "Detected suspicious process command line that uses base64 encoded input for execution with a shell";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// select * from bpf_process_events where cmdline like '%IyEvYmluL2Jhc2%' and cmdline like '%IyEvYmluL2Rhc2%' and cmdline like '%IyEvYmluL3pza%' and cmdline like '%IyEvYmluL2Zpc2%' and cmdline like '%IyEvYmluL3No%';
bool linux_base64_encoded_shebang_cli(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    if (cmdline.find("IyEvYmluL2Jhc2") != std::string::npos ||
        cmdline.find("IyEvYmluL2Rhc2") != std::string::npos ||
        cmdline.find("IyEvYmluL3pza") != std::string::npos ||
        cmdline.find("IyEvYmluL2Zpc2") != std::string::npos ||
        cmdline.find("IyEvYmluL3No") != std::string::npos)
    {
        std::stringstream ss;
        ss << "Detected the presence of a base64 version of the shebang in the commandline";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// T1059.004 - Bash Interactive Shell
// select * from bpf_process_events where path like '%bash%' and cmdline like '%-i%';
bool bash_interactive_shell(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if (path.find("bash") != std::string::npos && cmdline.find(" -i ") != std::string::npos)
    {
        std::stringstream ss;
        ss << "Detected execution of the bash shell with the interactive flag -i";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// TA0002 - Enable BPF Kprobes Tracing
// select * from bpf_process_events where (cmdline like '%echo 1 >%' and cmdline like '%/sys/kernel/debug/tracing/events/kprobes/%') OR (cmdline like '%/myprobe/enable%' or cmdline like '%/myretprobe/enable%');
bool enable_bpf_kprobes_tracing(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    if ((cmdline.find("echo 1 >") != std::string::npos && cmdline.find("/sys/kernel/debug/tracing/events/kprobes/") != std::string::npos) ||
        (cmdline.find("/myprobe/enable") != std::string::npos || cmdline.find("/myretprobe/enable") != std::string::npos))
    {
        std::stringstream ss;
        ss << "Detected common command used to enable bpf kprobes tracing";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// T1059.004 - BPFtrace Unsafe Option Usage
// select * from bpf_process_events where path like '%bpftrace%' and cmdline like '%--unsafe%';
bool bpftrace_unsafe_option_usage(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if (path.find("bpftrace") != std::string::npos && cmdline.find("--unsafe") != std::string::npos)
    {
        std::stringstream ss;
        ss << "Detected the usage of the unsafe bpftrace option";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// T1083 - Capabilities Discovery - Linux
// select * from bpf_process_events where path like '%getcap%' and (cmdline like '%/r%' or cmdline like '%-r%');
bool capabilities_discovery_linux(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if (path.find("getcap") != std::string::npos && (cmdline.find("/r") != std::string::npos || cmdline.find("-r") != std::string::npos))
    {
        std::stringstream ss;
        ss << "Detected the usage of the capabilities for discovery";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// T1592.004 - Cat Sudoers
// select * from bpf_process_events where (path like '%cat%' or path like '%grep%' or path like '%head%' or path like '%tail%' or path like '%more%') and cmdline like '%etc/sudoers%';
bool cat_sudoers(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if ((path.find("/cat") != std::string::npos || path.find("grep") != std::string::npos ||
         path.find("/head") != std::string::npos || path.find("/tail") != std::string::npos ||
         path.find("/more") != std::string::npos) &&
        cmdline.find(" /etc/sudoers") != std::string::npos)
    {
        std::stringstream ss;
        ss << "Detected the execution of a cat /etc/sudoers to list all users that have sudo rights";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// T1222.002 - Remove Immutable File Attribute
// select * from bpf_process_events where path like '%chattr%' and cmdline like '%-i%';
bool remove_immutable_file_attribute(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if (path.find("chattr") != std::string::npos && cmdline.find("-i") != std::string::npos)
    {
        std::stringstream ss;
        ss << "Detected usage of the 'chattr' utility to remove immutable file attribute";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// select * from bpf_process_events where (path like '%rm%' or path like '%shred%' or path like '%unlink%') and (cmdline like '%var/log%' or cmdline like '%var/spool/mail%' )
bool clear_linux_logs(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if ((path.find("/rm") != std::string::npos || path.find("/shred") != std::string::npos ||
         path.find("/unlink") != std::string::npos) &&
        (cmdline.find("/var/log") != std::string::npos || cmdline.find("/var/spool/mail") != std::string::npos))
    {
        std::stringstream ss;
        ss << "Detected usage of the 'chattr' utility to remove immutable file attribute";
        rule_event.metadata = ss.str();
...
// T1070.002 - Commands to Clear or Remove the Syslog
// select * from bpf_process_events where cmdline like '%rm /var/log/syslog%' or cmdline like '%rm -r /var/log/syslog%' or cmdline like '%rm -f /var/log/syslog%' or cmdline like '%rm -rf /var/log/syslog%' or cmdline like '%unlink /var/log/syslog%' or cmdline like '%unlink -r /var/log/syslog%' or cmdline like '%unlink -f /var/log/syslog%' or cmdline like '%unlink -rf /var/log/syslog%' or cmdline like '%mv /var/log/syslog%' or cmdline like '% >/var/log/syslog%' or cmdline like '% > /var/log/syslog%';

bool commands_to_clear_remove_syslog(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    if (cmdline.find("rm /var/log/syslog") != std::string::npos ||
        cmdline.find("rm -r /var/log/syslog") != std::string::npos ||
        cmdline.find("rm -f /var/log/syslog") != std::string::npos ||
        cmdline.find("rm -rf /var/log/syslog") != std::string::npos ||
        cmdline.find("unlink /var/log/syslog") != std::string::npos ||
        cmdline.find("unlink -r /var/log/syslog") != std::string::npos ||
        cmdline.find("unlink -f /var/log/syslog") != std::string::npos ||
        cmdline.find("unlink -rf /var/log/syslog") != std::string::npos ||
        cmdline.find("mv /var/log/syslog") != std::string::npos ||
        cmdline.find(" >/var/log/syslog") != std::string::npos ||
        cmdline.find(" > /var/log/syslog") != std::string::npos)
    {
        std::stringstream ss;
        ss << "Detected usage of the 'chattr' utility to remove immutable file attribute";
...
// T1552.001 - Copy Passwd Or Shadow From TMP Path
// select * from bpf_process_events where path like '%/cp%' and cmdline like '%/tmp/%' and (cmdline like '%passwd%' or cmdline like '%shadow%');

bool copy_password_shadow_tmp_path(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if (path.find("/cp") != std::string::npos &&
        cmdline.find("/tmp/") != std::string::npos &&
        (cmdline.find("passwd") != std::string::npos || cmdline.find("shadow") != std::string::npos))
    {
        std::stringstream ss;
        ss << " Detected the file 'passwd' or 'shadow' is copied from tmp path";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// T1053.003 - Remove Scheduled Cron Task/Job
// select * from bpf_process_events where path like '%crontab%' and cmdline like '%-r%';
bool remove_scheduled_cron_task(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    std::string path = process_event.entry.path;
    if (path.find("crontab") != std::string::npos &&
        cmdline.find("-r") != std::string::npos)
    {
        std::stringstream ss;
        ss << " Detected usage of the 'crontab' utility to remove the current crontab.";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
// T1496 - Linux crypto mining indicators
//  select * from bpf_process_events where cmdline like '%--cpu-priority=%' or cmdline like '%--donate-level=0%' or cmdline like '% -o pool.%' or cmdline like '% --nicehash%' or cmdline like '% --algo=rx/0 %' or cmdline like '%stratum+tcp://%' or cmdline like '%stratum+udp://%' or cmdline like '%sh -c /sbin/modprobe msr allow_writes=on%' or cmdline like '%LS1kb25hdGUtbGV2ZWw9%' or cmdline like '%0tZG9uYXRlLWxldmVsP%' or cmdline like '%tLWRvbmF0ZS1sZXZlbD%' or cmdline like '%c3RyYXR1bSt0Y3A6Ly%' or cmdline like '%N0cmF0dW0rdGNwOi8v%' or cmdline like '%zdHJhdHVtK3RjcDovL%' or cmdline like '%c3RyYXR1bSt1ZHA6Ly%' or cmdline like '%N0cmF0dW0rdWRwOi8v%' or cmdline like '%zdHJhdHVtK3VkcDovL%';

bool crypto_mining_indicators(const ProcessEvent &process_event, Event &rule_event)
{
    std::string cmdline = process_event.entry.cmdline;
    if (cmdline.find("--cpu-priority=") != std::string::npos ||
        cmdline.find("--donate-level=0") != std::string::npos ||
        cmdline.find(" -o pool.") != std::string::npos ||
        cmdline.find(" --nicehash") != std::string::npos ||
        cmdline.find(" --algo=rx/0 ") != std::string::npos ||
        cmdline.find("stratum+tcp://") != std::string::npos ||
        cmdline.find("stratum+udp://") != std::string::npos ||
        cmdline.find("sh -c /sbin/modprobe msr allow_writes=on") != std::string::npos ||
        cmdline.find("LS1kb25hdGUtbGV2ZWw9") != std::string::npos ||
        cmdline.find("0tZG9uYXRlLWxldmVsP") != std::string::npos ||
        cmdline.find("tLWRvbmF0ZS1sZXZlbD") != std::string::npos ||
        cmdline.find("c3RyYXR1bSt0Y3A6Ly") != std::string::npos ||
        cmdline.find("N0cmF0dW0rdGNwOi8v") != std::string::npos ||
        cmdline.find("zdHJhdHVtK3RjcDovL") != std::string::npos ||
        cmdline.find("c3RyYXR1bSt1ZHA6Ly") != std::string::npos ||
        cmdline.find("N0cmF0dW0rdWRwOi8v") != std::string::npos ||
        cmdline.find("zdHJhdHVtK3VkcDovL") != std::string::npos)
    {
        std::stringstream ss;
        ss << " Detected command line parameters or strings often used by crypto miners";
        rule_event.metadata = ss.str();
        return true;
    }
    return false;
...
//XZ and liblzma vulnerable packages

bool xzvuln(const ProcessEvent &process_event, Event &rule_event)
{
    std::string name = process_event.entry.name;
    std::string version = process_event.entry.version;

    if((name.find("xz-utils") != std::string::npos || name.find("xz-libs") != std::string::npos || name.find("liblzma") != std::string::npos) && (version.find("5.6.0") != std::string::npos || version.find("5.6.1") != std::string::npos))
        {
            std::stringstream ss;
            ss << "Potentially Vulnerable xz packages detected";
            rule_event.metadata = ss.str();
            return true;
        }
        return false;
